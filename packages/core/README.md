...
| `domain`        | `Habit`, `HabitView`, `HabitLog`, `HabitStartDateChange`, `ViewKind`, `TargetType` (types), `DEFAULT_HABIT_VIEWS`                                                 |
| `auth-schemas`  | `signUpSchema`, `signInSchema`, `emailSchema`, `passwordSchema`                                                                                                   |
| `habit-schemas` | `habitFormSchema`, `habitViewInputSchema`, `createLogSchema`, `updateLogSchema` — shared by the backend's request validation and the mobile app's habit/log forms |
